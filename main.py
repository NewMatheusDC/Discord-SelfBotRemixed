...
Use `{prefix}help <category>` to view specific commands."""
    ...
    categories = {
        ...
        "autocount": {
            ...
            f"{prefix}stopcount2 <channel-id> - Stop remote counting"
        ]
    }
}
...
@bot.command()
async def stopcount2(ctx, channel_id: str = None):
    try:
        await ctx.message.delete()
    except:
        pass
    ...
    if not hasattr(bot, 'active_counts') or channel_id not in bot.active_counts:
        await ctx.send("> No counting is running in the specified channel", delete_after=5)
        return
